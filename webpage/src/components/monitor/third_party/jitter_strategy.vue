  ...
  <div>
    ...
    <Modal
      ...
      <Form :model="dataForm" :rules="ruleDataForm" :label-width="110" >
        ...
        <FormItem label="策略:">
          对比 <InputNumber v-model="dataForm.hours_ago" ></InputNumber> 小时前(差值)&nbsp;&nbsp;
...
import {
  ...
  getOpenUserList,
  getTPJitterItem,
  createTPJitterStrategy,
  updateTPJitterStrategy,
  deleteTPJitterStrategy
} from '@/api/monitor/third_party.js'
import { sendNotice } from '@/libs/util.js'

export default {
  // @modify
  name: 'tp_jitter_strategy',
  data () {
    return {
      'modalAction': '',
      'searchVal': '',
      ...
      'tableColumns': [
        ...
        },
        {
          ...
          'sortType': 'asc'
          // render: (h, params) => {
          //   return h('span', {}, this.monitorItemList[params.row.monitor_item])
          // }
        },
        {
          'title': '策略',
          'width': 160,
          render: (h, params) => {
            return h('span', {}, `对比 ${params.row.hours_ago}小时前 ${params.row.op} ${params.row.alert_number}`)
          }
        },
        {
          'title': 'Note',
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteTPJitterStrategy(item.id).then(res => {
    ...
    createAction () {
      ...
      createTPJitterStrategy(commitForm).then(res => {
    ...
    updateAction () {
      let commitForm = this.handleCommitVal()
      updateTPJitterStrategy(this.dataForm.id, commitForm).then(res => {
