...
// @modify
import { getVpnInfo, deleteVpnInfo, updateVpnInfoMonitor } from '@/api/monitor/third_party'
      ...
      tableColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            let color = status === 'active' ? '#67C23A' : '#F56C6C'
            return h('Tag', {
              'props': {
                'color': color
              }
            ...
            return h('i-switch', {
              ...
              on: {
                'on-change': (status) => {
                  ...
                  updateVpnInfoMonitor(id, isMonitorNum).then(res => {
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteVpnInfo(item.id).then(res => {
