...
// @modify
import { getYuexinSmsInfo, deleteYuexinSmsInfo, updateYuexinSmsInfoMonitor, getTPChartData } from '@/api/monitor/third_party'
            ...
            return h('i-switch', {
              ...
              on: {
                'on-change': (status) => {
                  ...
                  updateYuexinSmsInfoMonitor(id, isMonitorNum).then(res => {
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteYuexinSmsInfo(item.id).then(res => {
