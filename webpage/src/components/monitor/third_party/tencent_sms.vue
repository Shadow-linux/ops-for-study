...
// @modify
import { getTencentSmsInfo, deleteTencentSmsInfo, updateTencentSmsInfoMonitor, getTPChartData } from '@/api/monitor/third_party'
            ...
            return h('i-switch', {
              ...
              on: {
                'on-change': (status) => {
                  ...
                  updateTencentSmsInfoMonitor(id, isMonitorNum).then(res => {
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteTencentSmsInfo(item.id).then(res => {
