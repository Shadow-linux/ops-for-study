...
// @modify
import { getDomainInfo, deleteDomainInfo, updateDomainInfoMonitor } from '@/api/monitor/third_party'
      ...
      tableColumns: [
        ...
        },
        {
          ...
          render: (h, params) => {
            ...
            let color = status === 'SUCCEED' ? '#67C23A' : '#F56C6C'
            return h('Tag', {
              'props': {
                'color': color
              }
            ...
            return h('i-switch', {
              ...
              on: {
                'on-change': (status) => {
                  ...
                  updateDomainInfoMonitor(id, isMonitorNum).then(res => {
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteDomainInfo(item.id).then(res => {
