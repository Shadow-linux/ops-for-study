...
// @modify
import {
  getWanweiyiyuanBankidentityInfo,
  deleteWanweiyiyuanBankidentityInfo,
  updateWanweiyiyuanBankidentityInfoMonitor,
  getTPChartData
} from '@/api/monitor/third_party'
import { sendNotice, lineChart } from '@/libs/util.js'
import { getDate } from '@/libs/tools.js'

export default {
  // @modify
  name: 'tp_wanweiyiyuan_bankid',
  props: [
    'selectItem'
  ],
  computed: {
  ...
  data () {
    return {
      // @modify
      searchColumn: 'api_name',
      // @modify
      monitorItem: 'wanweiyiyuan_bankid',
      chartModal: false,
      chartTitle: 'none',
      chartId: 0,
      chartDays: 7,
            ...
            return h('i-switch', {
              ...
              on: {
                'on-change': (status) => {
                  ...
                  updateWanweiyiyuanBankidentityInfoMonitor(id, isMonitorNum).then(res => {
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteWanweiyiyuanBankidentityInfo(item.id).then(res => {
    ...
    // 获取table datas
    getTableDatas () {
      getWanweiyiyuanBankidentityInfo().then(res => {
