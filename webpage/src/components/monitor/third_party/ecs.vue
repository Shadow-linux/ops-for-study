...
// @modify
import { getEcsInfo, deleteEcsInfo, updateEcsInfoMonitor } from '@/api/monitor/third_party'
            ...
            return h('i-switch', {
              ...
              on: {
                'on-change': (status) => {
                  ...
                  updateEcsInfoMonitor(id, isMonitorNum).then(res => {
  ...
  methods: {
    ...
    deleteItem () {
      this.selectValues.forEach(item => {
        deleteEcsInfo(item.id).then(res => {
